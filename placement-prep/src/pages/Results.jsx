...
export default function Results() {
    ...
    const handleToggleSkill = (skill) => {
        ...
        const updates = { skillConfidenceMap: newMap, readinessScore: newScore };
...
COMPANY INTEL:
Size: ${data.companyIntel?.sizeCategory}
Focus: ${data.companyIntel?.hiringFocus}

ROUND FLOW:
${data.roundMapping?.map((r, i) => `Round ${i + 1}: ${r.name} (${r.focus})`).join('\n')}
...
${data.plan.map(p => `${p.day} (${p.focus}):\n - ${p.tasks.join('\n - ')}`).join('\n\n')}
        ...
        const a = document.createElement('a'); a.href = url; a.download = `prep-plan.txt`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    };

    if (!data) return null;

    const weakSkills = Object.values(data.extractedSkills).flat().filter(s => (data.skillConfidenceMap?.[s] || 'practice') === 'practice').slice(0, 3);
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-10 pb-20 animate-in fade-in duration-700">
            <header className="flex items-center justify-between">
                ...
                <button onClick={handleDownload} className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-2xl text-slate-500 hover:text-primary hover:border-primary/20 transition-all font-bold">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
                    <div className="relative w-48 h-48 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-md border border-white/20 flex-shrink-0 text-center">
                        ...
                        <svg className="absolute inset-0 -rotate-90">
                            ...
                            <circle cx="96" cy="96" r="90" style={{ strokeDasharray: 565, strokeDashoffset: 565 - (data.readinessScore / 100) * 565, transition: 'stroke-dashoffset 0.5s ease-out' }} className="stroke-white fill-none stroke-[6] stroke-round" />
                        </svg>
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                        <ExportButton icon={copyStatus.plan ? <Check size={18} /> : <Copy size={18} />} label="Copy Plan" onClick={() => handleCopy('plan', 'Plan text...')} />
                    </div>
                </div>
            </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                ...
                                <div key={cat}>
                                    ...
                                    <div className="flex flex-wrap gap-3">
                                        {skills.map(s => {
                                            ...
                                            return (
                                                <button key={s} onClick={() => handleToggleSkill(s)} className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all border flex items-center gap-2 ${isKnown ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-slate-50 border-slate-200 text-slate-700 hover:border-primary/30'}`}>
                                                    {isKnown ? <Check size={14} /> : <Circle size={4} fill="currentColor" />}
                                                    {s}
                                                </button>
                                            );
                                        })}
                ...
                <div className="space-y-10">
                    {/* Company Intel Block */}
                    <div className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm group">
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <span className="font-bold text-slate-800">{data.companyIntel?.industry}</span>
                                    <span className="bg-slate-100 text-slate-500 px-2 py-0.5 rounded text-[10px] font-black">{data.companyIntel?.sizeCategory}</span>
                                </div>
                            </div>
                            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Typical Hiring Focus</h4>
                        ...
                        <div className="mt-8 pt-8 border-t border-slate-50 flex items-start gap-2 italic">
                            ...
                            <p className="text-[10px] text-slate-400 font-medium">Demo Mode: Company intel generated heuristically.</p>
                        </div>
                    </div>
